...
class DB(object):
    ...
    def __init__(
        ...
        if root is None:
            if download:
                self.root = os.path.expanduser("~/MSBDB24/MSBDB24-5")
            else:
    ...
    def download(self, progress: bool = True, suffix: str = ".zip"):
        ...
        try:
            ...
            if e.errno == errno.EEXIST:
                ...
                raise
        print("Downloading MSBDB 5-second .wav samples to %s..." % self.root)

        file_size = None
        req = Request(self.url, headers={"User-Agent": "msbdb_downloader"})
        u = urlopen(req)
        meta = u.info()
